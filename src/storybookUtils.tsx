...
export const generateMdFile = <T extends MdAttributes>(attributes: T, content?: string): Omit<MdFile<string>, "data"> & { data: T } => {
    return matter(generateMD(attributes, content)) as Omit<MdFile<string>, "data"> & { data: T }
}
...
export const generateMD = (attributes: MdAttributes, content?: string): string => {
...
export const generatePlaceholderContent = (size: number = 5): React.ReactNode => {
    return <>{Array.from({ length: size }).map((_, i) => <p key={i}>{faker.lorem.paragraph(i + 1)}</p>)}</>
}