 ...
 */
export const getAllInDirectory = async (directory: string) =>
  Promise.all(
    getFilesInDir(directory).map(
      async (filename): Promise<MdFile> =>
        await useReadMdFile(filename, directory),
    ),
  );
 ...
 */
export async function useReadMdFile(
  filename: string,
 ...
 * ## Heading 1{#heading-1}
 */
export const convertHeadingsToAnchorLinks = (markdown: string) =>
  markdown.replace(
    /(#{1,6})\s(.+)\n/gm,
...
export const useAllReferences = async () =>
  getAllInDirectory(referencesDirectory);
export const useAllEmployment = async () =>
  getAllInDirectory(employmentDirectory);
export const useAllEducation = async () =>
  getAllInDirectory(educationDirectory);
